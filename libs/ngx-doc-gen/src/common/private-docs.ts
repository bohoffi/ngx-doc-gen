...
export function isPublicDoc(doc: ApiDoc, docsPublicTag: string, docsPrivateTag: string) {
  ...
  if (_isEnforcedPublicDoc(doc, docsPublicTag)) {
    return true;
  }

  if (_hasDocsPrivateTag(doc, docsPrivateTag) || doc.name.startsWith('_')) {
    return false;
 ...
 */
export function getDocsPublicTag(doc: ApiDoc, docsPublicTag: string): Tag | undefined {
  return findJsDocTag(doc, docsPublicTag);
}
 ...
 */
function _hasDocsPrivateTag(doc: ApiDoc, docsPrivateTag: string): boolean {
  return hasJsDocTag(doc, docsPrivateTag);
}
 ...
 */
function _isEnforcedPublicDoc(doc: ApiDoc, docsPublicTag: string): boolean {
  return getDocsPublicTag(doc, docsPublicTag) !== undefined;
}
